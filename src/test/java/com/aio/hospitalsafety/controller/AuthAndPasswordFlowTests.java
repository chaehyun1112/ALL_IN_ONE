...
import static com.aio.hospitalsafety.common.SessionConstants.HOSPITAL_DOMAIN;
...
class AuthAndPasswordFlowTests {
    ...
    void showsHospitalLoginPage() throws Exception {
        mockMvc.perform(get("/"))
                ...
                .andExpect(content().string(org.hamcrest.Matchers.containsString("/domain")))
                .andExpect(content().string(org.hamcrest.Matchers.containsString("/css/auth/domain.css")))
                .andExpect(content().string(org.hamcrest.Matchers.containsString("class=\"domain-card\"")));
    }

    /** 비밀번호 변경 후 로그인 화면에 완료 안내가 표시되는지 확인한다. */
    @Test
    void showsPasswordChangedMessageOnLoginPage() throws Exception {
        when(hospitalService.findHospitalByDomain("HOSP01"))
                .thenReturn(new HospitalDto("HOSP01", "테스트병원"));
        mockMvc.perform(get("/login").sessionAttr(HOSPITAL_DOMAIN, "HOSP01").param("passwordChanged", ""))
                .andExpect(status().isOk())
    ...
    void movesToUserLoginAfterHospitalSelection() throws Exception {
        ...
        mockMvc.perform(get("/login")
                ...
                .andExpect(content().string(org.hamcrest.Matchers.containsString("관리자에게 문의")));
    }
    ...
    void showsStyledPasswordChangePage() throws Exception {
        mockMvc.perform(get("/user/password")
                ...
                .andExpect(content().string(org.hamcrest.Matchers.containsString("/css/auth/password-change.css")))
                .andExpect(content().string(org.hamcrest.Matchers.containsString("class=\"change-form\"")))
                .andExpect(content().string(org.hamcrest.Matchers.containsString("class=\"change-submit\"")))
                .andExpect(content().string(org.hamcrest.Matchers.containsString("/JS/auth/password-change.js")));
    }
    ...
    void changesAuthenticatedUserPassword(String selectedHospital) throws Exception {
        ...
        if (selectedHospital != null) session.setAttribute(HOSPITAL_DOMAIN, selectedHospital);
    ...
    /** 삭제된 사용자용 재설정 화면을 다시 노출하지 않는다. */
    @Test
    void doesNotRestorePasswordResetGuide() throws Exception {
        mockMvc.perform(get("/password/reset").with(user("USER01")))
