(function () {
  ...
  const checkUserIdButton = document.getElementById("checkUserIdBtn");
  ...
  checkUserIdButton.addEventListener("click", async () => {
    ...
    if (!userId) {
      userIdMessage.textContent = "아이디를 입력해 주세요.";
      userIdMessage.className = "username-message is-taken";
      return;
    }
    ...
    try {
      const response = await fetch(`/api/users/check-user-id?userId=${encodeURIComponent(userId)}`);
      if (!response.ok) {
      ...
      if (result.available) {
        ...
        userIdMessage.className = "username-message is-available";
      ...
      userIdMessage.textContent = "중복 확인 중 오류가 발생했습니다. 다시 시도해 주세요.";
      userIdMessage.className = "username-message is-taken";
      userIdChecked.value = "false";
    } finally {
  ...
  signupForm.addEventListener("submit", (event) => {
    if (userIdChecked.value !== "true") {
      ...
      userIdMessage.textContent = "아이디 중복 확인을 먼저 진행해 주세요.";
      userIdMessage.className = "username-message is-taken";
      userIdInput.focus();
    }
  });
